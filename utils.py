...
def create_friendly_repr_of_schedule_rule(rrule, exec_interval, exec_datetime, rule_timezone):
    ...
    time_str = start_hour
    # UNTIL and COUNT are mutually exclusive. Or ends on a dat, or ends after X
    # executions
    # UNTIL comes already localized
    until = rrule_values["UNTIL"] and parser.parse(rrule_values["UNTIL"]).strftime("%m/%d/%Y")
    ends_str = "ENDS ON "+until if until else "NEVER ENDS"
    count = rrule_values["COUNT"]
    ends_str = counts and "FOR "+count+" TIMES"
...
#create_friendly_repr_of_schedule_rule(rrule, exec_interval, exec_datetime, rule_timezone)
x = create_friendly_repr_of_schedule_rule("FREQ=MONTHLY;INTERVAL=1;UNTIL=20200320T025959Z;BYMONTHDAY=5", "Repeat", "2020-03-05 19:30:00", "America/Buenos_Aires")
print(x)
